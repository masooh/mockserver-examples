...
public class CallbackTest {
    ...
    public static class StatefulListCallback implements ExpectationCallback {
        ...
        public HttpResponse handle(HttpRequest httpRequest) {
            if (toMatcher(request().withMethod("POST")).matches(httpRequest)) {
                if (list.size() < 2) {
        ...
        private HttpRequestMatcher toMatcher(HttpRequest post) {
            return new MatcherBuilder().transformsToMatcher(post);
        }
    }
}
